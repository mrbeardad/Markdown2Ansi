...
/*************************************************************************************************/
#if !defined(MRBEARDAD_MINE_HPP_CSWS) &&                   \
    !defined(DISABLE_CLOSE_SYNC_WITH_STDIO) &&             \
...
#endif  // !defined(MRBEARDAD_MINE_HPP_CSWS) &&
        // !defined(DISABLE_CLOSE_SYNC_WITH_STDIO)
        // && (defined(_GLIBCXX_IOSTREAM) || defined(_IOSTREAM_))
        // && __cplusplus >= 201704L
...
// std::regex字面值
/*************************************************************************************************/
#if !defined(MRBEARDAD_MINE_HPP_REGEX) && \
    (defined(_GLIBCXX_REGEX) || defined(_REGEX_))
#define MRBEARDAD_MINE_HPP_REGEX
inline std::regex operator""_rgx(const char* str, size_t len) {
    return std::regex{str, len};
}

inline std::wregex operator""_rgx(const wchar_t* str, size_t len) {
...
class BasicSplit {
   ...
   public:
    ...
    explicit BasicSplit(const std::basic_string<CharT>& str,
                        CharT sep = -1,
                        bool keepSep = false)
        : view_{&str}, curIdx_{}, sep_{sep}, keepSep_{keepSep} {}

    const std::basic_string<CharT>& reset(
        const std::basic_string<CharT>& newView,
        char sep = -1,
        bool keepSep = false) {
        curIdx_ = 0;
        sep_ = sep;
    ...
    bool operator()(std::basic_string_view<CharT>& oneline) {
        if (sep_ == -1) {
            auto& viewStr = *view_;
            for (; curIdx_ < viewStr.size() && std::isspace(viewStr[curIdx_]);
                 ++curIdx_)
                ;
            if (curIdx_ == viewStr.size())
                return false;
            size_t nextSpace{curIdx_};
            for (; nextSpace < viewStr.size() &&
                   !std::isspace(viewStr[nextSpace]);
                 ++nextSpace)
                ;
            oneline = std::basic_string_view<CharT>{viewStr.data() + curIdx_,
                                                    nextSpace - curIdx_};
            curIdx_ = nextSpace;
            return true;
        }
        ...
        if (sepPos != std::basic_string<CharT>::npos) {
            ...
            oneline =
                std::basic_string_view<CharT>{view_->data() + curIdx_, len};
            curIdx_ = sepPos + 1;
            return true;
        } else if (curIdx_ < view_->size()) {
            oneline = std::basic_string_view<CharT>{view_->data() + curIdx_,
                                                    view_->size() - curIdx_};
            curIdx_ = sepPos;
            return true;
        } else {
            oneline =
                std::basic_string_view<CharT>{view_->data() + view_->size(), 0};
            return false;
        }
    }
};
